...
- (void)didTop {
    [self.view lc_showToastWithMessage:@"床前明月光，疑是地上霜。举头望明月，低头思故乡。" position:(LCToastPositionTop)];
}

- (void)didCenter {
    [self.view lc_showToastWithMessage:@"泉眼无声惜细流，树阴照水爱晴柔。小荷才露尖尖角，早有蜻蜓立上头。" position:(LCToastPositionCenter)];
}

- (void)didBottom {
    [self.view lc_showToastWithMessage:@"日照香炉生紫烟，遥看瀑布挂前川。飞流直下三千尺，疑是银河落九天。" position:(LCToastPositionBottom)];
}
...
- (void)startProgress:(NSTimer *)timer {
    ...
    [self.view lc_showProgress:self.count/10.0 message:@"下载中..."];
}
@end
